...
class ImageListener:
    def __init__(self, depth_image_topic, depth_info_topic, image_topic):
        self.bridge = CvBridge()
        self.sub_img = rospy.Subscriber(image_topic, msg_Image, self.imageCallback)
    ...
    def imageCallback(self, data):
    	self.image = self.bridge.imgmsg_to_cv2(data, "bgr8") #results in numpy  
    ...
    def imageDepthInfoCallback(self, cameraInfo):
        try:
            ...
            print(' ---> inrtinsics ',rs2.intrinsics())
            ...
            print('---> fx i fy = ', cameraInfo.K[0],' , ',cameraInfo.K[4])
...
def distance():
    	...
    	start_point=[self.pix[0]-10, self.pix[1]-10]
    	end_point=[self.pix[0]+10, self.pix[1]+10]
    	color=[250,10,100]
    	cv2.rectangle(cv_image,start_point,end_point, color, 2)
    	image_pub = rospy.Publisher("img_indentified",msg_Image, queue_size=10)
...
def main():
    ...
    print ('adf<sdvgfafgdfg')
    print('listener ', listener.pix)
...
if __name__ == '__main__':
    ...
    listener = ImageListener(depth_image_topic, depth_info_topic, image_topic)
